...
class MainFragment : Fragment(), OnTaskClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Passar entidade ciente do ciclo de vida
        taskViewModel.taskMutableLiveData.observe(requireActivity()){tasks ->
            taskList.clear() // Não fazer isso para app reais com listas grandes
            tasks.forEachIndexed{index, task ->
                taskList.add(task)
                tasksAdapter.notifyItemChanged(index) // notificar o adapter que um indice foi alterado
            }
        }
    ...
    override fun onCreateView(
        ...
        fmb = FragmentMainBinding.inflate(inflater, container, false).apply {
            ...
            // Ação que permitir sair de main fragment para outro fragment (criar)
